...
import { TimelineEvent, UpdateTimelineEventRequest, Location } from '../services/api-types';
...
export default function EditStoryModal({ isOpen, onClose, onSuccess, event }: EditStoryModalProps) {
  ...
  useEffect(() => {
    if (isOpen && event) {
      ...
      if (event.locations && event.locations.length > 0) {
        setSelectedLocationIds(event.locations.map(loc => loc.id));
      } else {
        setSelectedLocationIds([]);
      }
      
      // 设置已选照片和封面
      if (event.photos_videos && event.photos_videos.length > 0) {
        setSelectedPhotoIds(event.photos_videos.map(photo => photo.id));
        
        // 如果有封面，设置封面ID
        if (event.cover_url) {
          const coverPhoto = event.photos_videos.find(photo => 
            photo.media_url === event.cover_url);
          if (coverPhoto) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const eventData: UpdateTimelineEventRequest = {
        event_id: event.id,
        couple_id: coupleId,
        title,
      ...
      await eventService.updateEvent(Number(event.id), eventData);
      ...
      setError(
        err.response?.data?.message || 
        (language === 'zh' ? '更新故事失败，请稍后重试' : 'Failed to update story, please try again later')
      );
    } finally {
      setIsLoading(false);
    }
      ...
      <div className="step-content">
        <div className="form-group" style={formGroupStyle}>
          ...
          <input
            ...
            placeholder={language === 'zh' ? '例如：第一次约会' : 'Example: Our First Date'}
        ...
        <div className="form-group" style={formGroupStyle}>
          ...
          <textarea
            ...
            placeholder={language === 'zh' ? '描述一下这个故事...' : 'Describe your story...'}
            style={{
    ...
    <div className="modal-overlay" style={{ backgroundColor: 'rgba(0, 0, 0, 0.7)' }}>
      <div className="modal-container wide-modal" style={{
        ...
        <div className="modal-header" style={{ borderBottomColor: 'var(--border-primary)' }}>
          <h2 style={{ color: 'var(--text-primary)' }}><T zh="编辑回忆" en="Edit Story" /></h2>
